...
const PVNM_CANVAS_ASPECT_RATIO = 3 / 2;
const PVNM_CANVAS_WIDTH = 720;
const PVNM_CANVAS_HEIGHT = 480;
let _pvnmCanvasContainApplying = false;
let _pvnmCanvasFitEnabled = false;
...
async function launchPyxel(params) {
  ...
  const pyodideVersionMatch = PYODIDE_URL.match(/v([\d.]+)\//);
  const pyodideVersion = pyodideVersionMatch ? pyodideVersionMatch[1] : "0.29.3";
...
const _setStyleImportant = (elem, name, value) => {
  if (
    elem.style.getPropertyValue(name) !== value ||
    elem.style.getPropertyPriority(name) !== "important"
  ) {
    elem.style.setProperty(name, value, "important");
  }
};

const _applyPvnmCanvasContain = () => {
  if (_pvnmCanvasContainApplying) {
    return;
  }

  const canvas = document.querySelector("canvas#canvas");
  const pyxelScreen = document.querySelector("div#pyxel-screen");
  if (!canvas || !pyxelScreen) {
    return;
  }
  ...
  const canvasScale = Math.min(
    viewportWidth / PVNM_CANVAS_WIDTH,
    viewportHeight / PVNM_CANVAS_HEIGHT,
  );
  const displayWidth = PVNM_CANVAS_WIDTH * canvasScale;
  const displayHeight = PVNM_CANVAS_HEIGHT * canvasScale;
  const displayLeft = Math.max(0, (viewportWidth - displayWidth) / 2);
  const displayTop = Math.max(0, (viewportHeight - displayHeight) / 2);
  const unscaledLeft = (viewportWidth - PVNM_CANVAS_WIDTH) / 2;
  const unscaledTop = (viewportHeight - PVNM_CANVAS_HEIGHT) / 2;

  _pvnmCanvasContainApplying = true;
  try {
    _setStyleImportant(pyxelScreen, "position", "fixed");
    _setStyleImportant(
      pyxelScreen,
      "left",
      `${_pvnmCanvasFitEnabled ? displayLeft : unscaledLeft}px`,
    );
    _setStyleImportant(
      pyxelScreen,
      "top",
      `${_pvnmCanvasFitEnabled ? displayTop : unscaledTop}px`,
    );
    _setStyleImportant(pyxelScreen, "width", `${PVNM_CANVAS_WIDTH}px`);
    _setStyleImportant(pyxelScreen, "height", `${PVNM_CANVAS_HEIGHT}px`);
    _setStyleImportant(pyxelScreen, "background-color", "#000");
    _setStyleImportant(pyxelScreen, "overflow", "hidden");
    _setStyleImportant(pyxelScreen, "transform-origin", "left top");
    _setStyleImportant(
      pyxelScreen,
      "transform",
      _pvnmCanvasFitEnabled ? `scale(${canvasScale})` : "none",
    );

    if (canvas.width !== PVNM_CANVAS_WIDTH) {
      canvas.width = PVNM_CANVAS_WIDTH;
    }
    if (canvas.height !== PVNM_CANVAS_HEIGHT) {
      canvas.height = PVNM_CANVAS_HEIGHT;
    }
    if (canvas.getAttribute("width") !== String(PVNM_CANVAS_WIDTH)) {
      canvas.setAttribute("width", String(PVNM_CANVAS_WIDTH));
    }
    if (canvas.getAttribute("height") !== String(PVNM_CANVAS_HEIGHT)) {
      canvas.setAttribute("height", String(PVNM_CANVAS_HEIGHT));
    }
    _setStyleImportant(canvas, "position", "absolute");
    _setStyleImportant(canvas, "left", "0");
    _setStyleImportant(canvas, "top", "0");
    // Keep the backing store at 720x480, while the official #pyxel-screen
    // container controls the displayed size.
    _setStyleImportant(canvas, "width", "100%");
    _setStyleImportant(canvas, "height", "100%");
    _setStyleImportant(canvas, "aspect-ratio", `${PVNM_CANVAS_WIDTH} / ${PVNM_CANVAS_HEIGHT}`);
    _setStyleImportant(canvas, "max-width", "none");
    _setStyleImportant(canvas, "max-height", "none");
    _setStyleImportant(canvas, "transform-origin", "left top");
    _setStyleImportant(canvas, "transform", "none");
    _setStyleImportant(canvas, "object-fit", "contain");
    _setStyleImportant(canvas, "image-rendering", "pixelated");
    if (!window.__PVNM_CANVAS_METRICS_LOGGED__) {
      window.__PVNM_CANVAS_METRICS_LOGGED__ = true;
      const canvasRect = canvas.getBoundingClientRect();
      const screenRect = pyxelScreen.getBoundingClientRect();
      console.log("[PVNM_CANVAS] metrics", JSON.stringify({
        window: [viewportWidth, viewportHeight],
        screen: [screenRect.width, screenRect.height, screenRect.left, screenRect.top],
        canvasAttr: [canvas.width, canvas.height],
        canvasClient: [canvas.clientWidth, canvas.clientHeight],
        canvasRect: [canvasRect.width, canvasRect.height, canvasRect.left, canvasRect.top],
        fitEnabled: _pvnmCanvasFitEnabled,
        scale: canvasScale,
      }));
    }
  } finally {
    setTimeout(() => {
      _pvnmCanvasContainApplying = false;
    }, 0);
  }
};

window.PVNM_APPLY_CANVAS_FIT = () => {
  _pvnmCanvasFitEnabled = true;
  window.__PVNM_CANVAS_METRICS_LOGGED__ = false;
  _schedulePvnmCanvasContain();
};

const _installPvnmCanvasContainGuard = () => {
  const canvas = document.querySelector("canvas#canvas");
  if (!canvas || canvas.dataset.pvnmContainGuard === "1") {
    return;
  }

  canvas.dataset.pvnmContainGuard = "1";
  const observer = new MutationObserver(() => {
    if (!_pvnmCanvasContainApplying) {
      requestAnimationFrame(_applyPvnmCanvasContain);
    }
  });
...
const _schedulePvnmCanvasContain = () => {
  _installPvnmCanvasContainGuard();
  _applyPvnmCanvasContain();
  for (const delay of [50, 250, 750, 1500, 3000, 6000, 10000]) {
    setTimeout(_applyPvnmCanvasContain, delay);
  }
};
